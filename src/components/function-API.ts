...
export const  getFullDataUser = async (user:userType) => {
  if ((user.projects.length!==0)&&((user.projects as string[]).filter((pr)=>pr!==undefined||null).length!==0)) {
...
export const  addProjectToUser = async ( user:userType, project:bodyProjectType) =>{
 ...
 (user.projects as ProjectType[]).push(newProject);
 const array=  (user.projects as ProjectType[]);
 const projecta:string[] =array.map((project:ProjectType) => {
  console.log(project._id);
  return   project._id});
 const {_id, login, password, name, surname} =user;
 let changedUser= await changeUser(_id, {login, password, name, surname,  projects:projecta});
...
export const  addColumnToUser = async ( user:userType, project:ProjectType, column:bodyProjectColumnsType) =>{
  ...
  (project.columns as ProjectColumnsType[]).push(newColumn);
